...
Bitnami charts can be used with [Kubeapps](https://kubeapps.com/) for deployment and management of Helm Charts in clusters. This chart has been tested to work with NGINX Ingress, cert-manager, Vector and Prometheus on top of the [BKPR](https://kubeprod.io/).
...
| `postgresqlRunPermissions.enabled`            | Chown the writable `/var/run/postgresql` socket dir so PostgreSQL can use it. Automatically forced off when `global.openshift.enabled` is `true` (cannot be re-enabled there via values) | `true`                                                      |
| `postgresqlRunPermissions.securityContext.runAsUser` | User ID for the `init-postgresql-run` init container                                                            | `0`                                                         |
