...
| `postgresqlRunPermissions.enabled`            | Chown the writable `/var/run/postgresql` socket dir so PostgreSQL can use it. Disable on clusters whose SCC/PSP disallows a root or capability-adding init container | `true`                                                      |
