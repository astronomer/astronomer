...
| `pre_helm_scripts`  | no       | ordered list of executable scripts (repo-relative) run *after* the cluster is created but *before* `helm install` — for setup the chart's install-time values depend on (e.g. creating a private-CA Secret named in `global.privateCaCerts`). Each runs with `KUBECONFIG` pointed at the fresh cluster and the helper-tools PATH, and must be executable (shebang + `chmod +x`). |
