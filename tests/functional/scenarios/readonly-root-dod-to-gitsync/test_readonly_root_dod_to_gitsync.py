...
DagOnlyDeployment (dag_deploy) -> GitSync via upsertDeployment -- the same call that both applies the
...
GitSync topology enforces readOnlyRootFilesystem afterward.
...
SCENARIO_LABEL = "readonly-root-dod-to-gitsync"
...
def test_dag_deployment_type_switch_disables_readonly_root(deployment, _k8s_apps_v1_client_module, _k8s_core_v1_client_module):
    ...
    switches the same deployment DagOnlyDeployment (dag_deploy) -> GitSync via upsertDeployment on the same
