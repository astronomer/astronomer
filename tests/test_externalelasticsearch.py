...
secret = base64.b64encode("sample-secret".encode()).decode()
...
class TestExternalElasticSearch:
    ...
    def test_externalelasticsearch_with_secret(self, kube_version):
        ...
        docs = render_chart(
            ...
            show_only=["charts/external-es-proxy/templates/external-es-proxy-deployment.yaml"],
        )
    ...
    def test_externalelasticsearch_with_awsSecretName(self, kube_version):
        ...
        assert doc["metadata"]["name"] == "RELEASE-NAME-external-es-proxy"
        expected_env = [
            {'name': 'AWS_ACCESS_KEY_ID', 'valueFrom': {'secretKeyRef': {'name': 'awssecret', 'key': 'aws_access_key'}}}, 
            {'name': 'AWS_SECRET_ACCESS_KEY', 'valueFrom': {'secretKeyRef': {'name': 'awssecret', 'key': 'aws_secret_key'}}},
    ...
    def test_externalelasticsearch_with_awsIAMRole(self, kube_version):
        ...
        expected_env = [{
            'name': 'ENDPOINT', 
            'value': 'https://esdemo.example.com'
            }
        ]
