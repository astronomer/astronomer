...
class TestContainerResourcesAndServiceAccount:
    ...
    def test_pod_service_account_name(self, config_data):
        ...
        assert isinstance(service_account_name, str), f"Pod {pod_data['name']} in chart {pod_data['chart']} serviceAccountName must be a string"
    ...
    def test_containers_have_resources_section(self, config_data):
        ...
        for i, container in enumerate(containers):
            ...
            assert 'resources' in container, f"Container '{container_name}' in {pod_data['kind']} {pod_data['name']} (chart: {pod_data['chart']}) missing resources key"
            ...
            assert limits or requests, f"Container '{container_name}' in {pod_data['kind']} {pod_data['name']} (chart: {pod_data['chart']}) has resources section but missing both limits and requests"
    ...
    def test_init_containers_have_resources_section(self, config_data):
        ...
        for i, container in enumerate(init_containers):
            ...
            assert resources, f"Init container '{container_name}' in {pod_data['kind']} {pod_data['name']} (chart: {pod_data['chart']}) missing resources section"

            limits = resources.get('limits', {})
            assert limits, f"Init container '{container_name}' in {pod_data['kind']} {pod_data['name']} (chart: {pod_data['chart']}) missing resource limits section"

            requests = resources.get('requests', {})
            assert requests, f"Init container '{container_name}' in {pod_data['kind']} {pod_data['name']} (chart: {pod_data['chart']}) missing resource requests section"