...
class TestIngress:
    ...
    def test_astro_ui_per_host_ingress(self, mode, expected, kube_version):
        docs = render_chart(
            kube_version=kube_version,
            values={"global": {"perHostIngress": {"enabled": True}, "plane": {"mode": mode}, "podLabels": self.labels}},
            show_only=[
    ...
    def test_single_ingress_per_host(self, kube_version):
        default_docs = render_chart(values={"global": {"perHostIngress": {"enabled": True, "podLabels": self.labels}}})
