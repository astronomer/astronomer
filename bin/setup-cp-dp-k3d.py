...
def _ensure_nginx_resolver_ipv6_off(*, context: str, namespace: str, configmap_name: str) -> None:
    ...
    the already-generated-and-hand-rolled ConfigMap directly with `http-snippet` — a raw key the
    controller reads regardless of whether this chart's templates parameterize it — to add a
    second `resolver` line with `ipv6=off`, which stops it from ever asking for AAAA. The
    controller watches its ConfigMap and hot-reloads nginx.conf on change, so no pod restart is
    needed. `kubectl patch --type=merge` is idempotent, consistent with this script's "safe to
    re-run" design.

    Local-dev-only shim: this is a workaround in setup tooling, not a fix to the chart itself —
    see the same annotation-scoped workaround (`nginx.ingress.kubernetes.io/server-snippet`) that
    unblocked this live, per-Ingress, before this function existed.
    """
    dns_ip = _cluster_dns_clusterip(context)
    patch = json.dumps({"data": {"http-snippet": f"resolver {dns_ip} valid=30s ipv6=off;"}})
    _run(
        [
            "kubectl",
            "--context",
