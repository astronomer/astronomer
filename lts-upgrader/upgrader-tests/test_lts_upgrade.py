...
def test_upgrade():
    """
    Functional test for the LTS to LTS upgrade (0.25 to 0.28)
    """
    ...
    upgrade_manifest_path = Path(
        git_root_dir
        / "lts-upgrader/upgrader/25-to-28/manifests/upgrade-0.25-to-0.28.yaml"
    )
    rollback_manifest_path = Path(
        git_root_dir
        / "lts-upgrader/upgrader/25-to-28/manifests/rollback-0.25-to-0.28.yaml"
    )
    ...
    ), "Expected 0.25 to be installed before upgrade"
    assert (
    ...
    print("Performing upgrade from 0.25 to 0.28")
    ...
    assert (
        "0.28" in helm_history.split("\n")[-2]
    ), "Expected version 0.28 to be installed after upgrade"
    assert (
        "deployed" in helm_history.split("\n")[-2]
    ...
    assert (
        "0.25" in helm_history.split("\n")[-2]
    ), "Expected version 0.25 to be installed after rollback"
    assert (
        "deployed" in helm_history.split("\n")[-2]
